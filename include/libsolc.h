...
/// Opaque long-lived compiler session.
typedef struct solidity_session solidity_session;
...
/// Creates an in-memory compiler session that retains reusable frontend and
/// backend state across Standard JSON requests.
///
/// @returns A session handle, or NULL if it could not be allocated.
solidity_session* solidity_session_create(void) SOLC_NOEXCEPT;

/// Compiles one Standard JSON request through a long-lived session.
///
/// The returned pointer follows the same ownership rules as solidity_compile().
/// Calls may run concurrently, but the session must not be destroyed until all
/// calls have returned. A source callback must not call this function with the
/// same session: a direct reentrant call returns NULL while the outer compile
/// remains valid. Calling through a different session remains supported.
char* solidity_session_compile(solidity_session* _session, char const* _input, CStyleReadFileCallback _readCallback, void* _readContext) SOLC_NOEXCEPT;

/// Destroys a compiler session. Passing NULL is allowed.
void solidity_session_destroy(solidity_session* _session) SOLC_NOEXCEPT;

/// Frees up any allocated memory.
///
/// NOTE: pointers returned by solidity_compile(), solidity_session_compile(),
/// or solidity_alloc() are invalid after calling this. Live compiler sessions
/// and their retained state are not destroyed or invalidated.
